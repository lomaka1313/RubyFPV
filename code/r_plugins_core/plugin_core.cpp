...
const char* g_szPluginNameExample = "Usb camera Core Plugin";
const char* g_szUIDExample = "4C1D7AE1-71B4-4136-9BAF-CEC92C81A31F";
...
{
   core_plugin_util_log_line("USB camera plugin request capabilities.");

   return CORE_PLUGIN_CAPABILITY_VIDEO_STREAM;
}
...
int core_plugin_init(u32 uRuntimeLocation, u32 uAllocatedCapabilities)
{
   ...
   core_plugin_util_log_line("Usb camera plugin init");
   return 0;
}
...
{
   core_plugin_util_log_line("Usb camera plugin uninit");
}

int core_plugin_on_get_video_stream_source_type() {
    return CORE_PLUGIN_VIDEO_STREAM_SOURCE_USB;
}
